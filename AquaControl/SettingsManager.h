...
public:
    void begin() {
        ...
        strlcpy(_settings.startTime, start.c_str(), sizeof(_settings.startTime));
        ...
        strlcpy(_settings.co2OnTime, co2On.c_str(), sizeof(_settings.co2OnTime));
        String co2Off = _prefs.getString("co2OffT", "22:40");
        strlcpy(_settings.co2OffTime, co2Off.c_str(), sizeof(_settings.co2OffTime));
        
        String fanOn = _prefs.getString("fanOnT", "10:00");
        strlcpy(_settings.fanOnTime, fanOn.c_str(), sizeof(_settings.fanOnTime));
        String fanOff = _prefs.getString("fanOffT", "18:00");
        strlcpy(_settings.fanOffTime, fanOff.c_str(), sizeof(_settings.fanOffTime));
    ...
    bool updateFromJson(JsonObject doc) {
        ...
        if (doc.containsKey("currentBrightness")) { _settings.currentBrightness = constrain((int)doc["currentBrightness"], 0, 100); changed = true; }
        
        if (doc.containsKey("startTime")) { strlcpy(_settings.startTime, doc["startTime"], sizeof(_settings.startTime)); changed = true; }
        if (doc.containsKey("photoperiod")) { _settings.photoperiod = constrain((int)doc["photoperiod"], 0, 24); changed = true; }
        if (doc.containsKey("maxBrightness")) { _settings.maxBrightness = constrain((int)doc["maxBrightness"], 0, 100); changed = true; }
        if (doc.containsKey("isDimmerEnabled")) { _settings.isDimmerEnabled = doc["isDimmerEnabled"]; changed = true; }
        if (doc.containsKey("sunriseMins")) { _settings.sunriseMins = constrain((int)doc["sunriseMins"], 0, 1440); changed = true; }
        if (doc.containsKey("sunsetMins")) { _settings.sunsetMins = constrain((int)doc["sunsetMins"], 0, 1440); changed = true; }
        if (doc.containsKey("recoveryMins")) { _settings.recoveryMins = constrain((int)doc["recoveryMins"], 0, 1440); changed = true; }
        ...
        if (doc.containsKey("fanSpeed")) { _settings.fanSpeed = constrain((int)doc["fanSpeed"], 0, 100); changed = true; }

        if (doc.containsKey("colorSpectrum")) {
            JsonObject col = doc["colorSpectrum"];
            if (col.containsKey("w")) _settings.colorW = constrain((int)col["w"], 0, 100);
            if (col.containsKey("r")) _settings.colorR = constrain((int)col["r"], 0, 100);
            if (col.containsKey("g")) _settings.colorG = constrain((int)col["g"], 0, 100);
            if (col.containsKey("b")) _settings.colorB = constrain((int)col["b"], 0, 100);
            changed = true;
        }

        // Support for flat color keys sent by the web app
        if (doc.containsKey("colorW")) { _settings.colorW = constrain((int)doc["colorW"], 0, 100); changed = true; }
        if (doc.containsKey("colorR")) { _settings.colorR = constrain((int)doc["colorR"], 0, 100); changed = true; }
        if (doc.containsKey("colorG")) { _settings.colorG = constrain((int)doc["colorG"], 0, 100); changed = true; }
        if (doc.containsKey("colorB")) { _settings.colorB = constrain((int)doc["colorB"], 0, 100); changed = true; }
    ...
    void processLazyFlashSave() {
        ...
        _prefs.putString("devName", _settings.deviceName);
        ...
        _prefs.putString("startTime", _settings.startTime);
        ...
        _prefs.putString("co2OnT", _settings.co2OnTime);
        _prefs.putString("co2OffT", _settings.co2OffTime);
        _prefs.putString("fanOnT", _settings.fanOnTime);
        _prefs.putString("fanOffT", _settings.fanOffTime);
