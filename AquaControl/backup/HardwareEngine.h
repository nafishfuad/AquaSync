    ...
    void execute(TankSettings& settings, bool forceEval = false, bool debugLog = false) {
        ...
        if (timeValid && (nowMillis - _lastBreadcrumbTick >= 60000)) {
            ...
            if (settings.lastTrackedDay != 0 && settings.lastTrackedDay != timeinfo->tm_mday) {
                for (int i = 29; i > 0; i--) settings.activeMinutesHistory[i] = settings.activeMinutesHistory[i-1];
                uint16_t yTotal = 0;
                for(int i=0; i<24; i++) { yTotal += settings.activeMinutesToday[i]; settings.activeMinutesToday[i] = 0; settings.awakeMinutesToday[i] = 0; }
                settings.activeMinutesHistory[0] = yTotal;
                settings.totalLoadSheddingToday = 0; settings.lightLoadSheddingToday = 0;
                settings.lastTrackedDay = timeinfo->tm_mday;
                saveAnalyticsVault(settings);
            } else if (settings.lastTrackedDay == 0) settings.lastTrackedDay = timeinfo->tm_mday;
