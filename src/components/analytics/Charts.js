...
export function renderCharts(container, analyticsData) {
  ...
  const createChartCard = (title, statsArray, chartConfigBuilder) => {
    ...
    statsArray.forEach((stat, index) => {
      ...
      if (statsArray.length === 2 && index === 1) alignClass = "text-right";
        ...
        if (index === 1)
          alignClass = "text-center border-x border-gray-800/60 px-2";
        if (index === 2) alignClass = "text-right";
      }

      let wrapperClass = alignClass;
      let onClickAttr = "";
      if (stat.onClick) {
        wrapperClass +=
          " cursor-pointer hover:bg-black/5 hover:dark:bg-white/5 rounded-lg p-2 transition-colors -m-2";
        onClickAttr = `onclick="${stat.onClick}"`;
      }

      statsHTML += `
                <div class="${wrapperClass}" ${onClickAttr}>
  ...
  createChartCard(
    ...
    [
      {
        label: "Total Active",
        value: analyticsData.today.totalActive,
        color: "text-white",
      },
      {
        label: "Load Shedding",
  ...
  createChartCard(
    ...
    [
      {
        label: "Total Active",
        value: analyticsData.week.totalActive,
        color: "text-white",
      },
      {
        label: "Daily Avg",
        value: analyticsData.week.avgLight,
        color: "text-aqua",
      },
      {
        label: "Load Shedding",
  ...
  createChartCard(
    ...
    [
      {
        label: "Total Active",
        value: analyticsData.month.totalActive,
        color: "text-white",
      },
      {
        label: "Daily Avg",
        value: analyticsData.month.avgLight,
